...
public class FollowerAdapter extends ArrayAdapter<FollowingLikeData> {
    ...
    public FollowerAdapter(Context context, ArrayList<FollowingLikeData> values) {
        super(context, R.layout.follower_list_item, values);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // Get item list view
        if (rowView == null) {
            rowView = mInflater.inflate(R.layout.follower_list_item, null);
            ...
            Config.scaleLayout(mContext, "follower", rowView);
            Config.processEmojiconViewHeight(mContext, "follower", viewHolder.textName);
        ...
        if (data != null) {
            ...
            if (SkilledManager.mParseUserMap.containsKey(objectId)) {
                ...
                loadPhotoImage(viewHolder, user);
            } else {
                viewHolder.imagePhoto.setImageDrawable(mDefaultAvatarDrawable);
            }

            Typeface typeFace = Typeface.createFromAsset(getContext().getAssets(), "fonts/AvenirNext-Regular.otf");
